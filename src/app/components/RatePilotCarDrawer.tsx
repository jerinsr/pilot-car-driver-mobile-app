...
interface RatePilotCarDrawerProps {
  pilotCarName: string;
  jobId: string;
  tripId?: string;
  location: string;
  onClose: () => void;
  onSubmit: (rating: SubmittedRating) => void;
}
...
export function RatePilotCarDrawer({
  pilotCarName,
  jobId,
  tripId,
  location,
  ...
  const handleSubmit = () => {
    ...
    const rating: SubmittedRating = {
      jobId,
      tripId,
      pilotCarId: pilotCarName, // In real app, would use actual ID
      driverId: "current-driver-id", // In real app, would use actual driver ID
    ...
    if (overallRating <= 3) {
      console.log("Low rating alert triggered for admin:", {
        ...
        message: `Pilot Car ${pilotCarName} received a rating of ${overallRating.toFixed(1)}★ for Job ${jobId}. Please review the feedback.`,
      });
    }
            ...
            <DrawerFooter className="border-t border-gray-100 pt-4 pb-6">
              <div className="flex gap-3 w-full">
                ...
                <Button
                  ...
                  disabled={!allRated}
                  className="flex-1 h-11 bg-[#f89823] hover:bg-[#e08820] text-white font-medium disabled:opacity-40 disabled:cursor-not-allowed disabled:bg-gray-300 disabled:text-gray-500"
                >
                  Submit Rating
                </Button>
              </div>
